...
function CardInfo({budgetList}) {
    ...
    useEffect(() => {
        if (budgetList && budgetList.length > 0 && budgetList.length !== prevLength) {
            CalculateCardInfo();
            setPrevLength(budgetList.length);
        }
    }, [budgetList, prevLength]);
  ...
  return (
    <div>
    {budgetList?.length>0?
    <div className='mt-7 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3  gap-5 cursor-pointer '>
